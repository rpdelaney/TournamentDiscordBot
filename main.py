...
@bot.tree.command(name="setup",
                  description="Creates categories and Tournament Staff role.")
...
async def pair_teams(interaction: discord.Interaction,
                     team1: discord.Role,
                     team2: discord.Role):

    scheduling_category = discord.utils.get(interaction.guild.categories,
                                            name="Scheduling")

    # Server doesn't have a Scheduling category, abort.
    if not scheduling_category:
        await interaction.response.send_message(
            'Your Discord is not setup, run **/setup** first.',
            ephemeral=True)
        return
